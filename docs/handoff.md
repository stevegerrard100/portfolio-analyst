...
| `sale_value` | `abs(walletImpact.netValue)` from T212 — total GBP proceeds; `None` if missing or zero |
...
3. **Sector leaders** (`fetch_sector_leaders()`) — top 5 large-cap, volume-confirmed stocks per sector by 1-week performance.

Also computes a **portfolio alignment score**. The `finviz_performance` list drives the Today's Brief sector heatmap.

**`fetch_sector_leaders()`** — for each of the 11 `FINVIZ_SECTORS` (the exact Finviz sector filter values, which double as the keys used by `finviz_performance`/`_sector_heatmap`), fetches the top 5 stocks by 1-week performance. Two lightweight single-page Finviz requests per sector: `finvizfinance.screener.performance.Performance` ordered by `"Performance (Week)"` descending with `limit=5` (Finviz's own sort does the ranking) over the filtered pool, then `finvizfinance.screener.overview.Overview` scoped to just those 5 tickers (via the `ticker=` param) for company name and market cap.

Filters (`FINVIZ_SECTOR_LEADER_FILTER`): `Market Cap.: +Large (over $10bln)` (Finviz's cumulative large-cap bucket, not restricted to S&P 500 constituents), `Country: USA`, `Industry: Stocks only (ex-Funds)` (excludes ETFs/closed-end funds — otherwise leveraged/derivative ETFs still show up tagged with a regular GICS sector), `Relative Volume: Over 1` (above-average volume, so the 1-week movers are volume-confirmed rather than thin/illiquid spikes). The volume filter is a pre-filter on the candidate pool, not a secondary sort key — ranking is purely by 1-week % performance within that pool. Because of the market cap floor, `market_cap_bucket` (`small`/`mid`/`large`, same $2B/$10B boundaries as `high_growth_screener.py`'s universe cap ranges) will in practice almost always read `"large"`; the field is kept for future-proofing rather than removed. Portfolio holdings are **not** excluded.

**Rate limiting**: Finviz rate-limits aggressively across 11 back-to-back sector calls. `fetch_sector_leaders()` sleeps `_SECTOR_LEADER_SLEEP_SECONDS` (1.5s) between each sector's fetch. `_screener_view_with_429_retry()` wraps each individual `screener_view()` call and, on an HTTP 429, backs off `_SECTOR_LEADER_429_BACKOFF_SECONDS` (5s) and retries once before giving up on that sector; any other HTTP error (or a second 429) propagates. Each sector's fetch is wrapped in try/except at the `fetch_sector_leaders()` level too — a failed sector gets `[]` and a logged warning rather than aborting the whole call.

Result shape: `{sector_name: [{ticker, company_name, perf_1w, market_cap_bucket}, ...]}`. Cached for 24 hours (`cache/sector_leaders.json`) — but only if at least `_SECTOR_LEADER_MIN_SUCCESSFUL_SECTORS` (6 of 11) sectors returned non-empty data; below that threshold the write is skipped and a warning logged, so a burst of 429s doesn't lock in a mostly-empty result for the full 24h TTL (the next run retries fresh instead). `_parse_perf_col()` (module-level, shared with `fetch_sector_performance()`) normalises Finviz's mixed percentage formats.
...
### Screener candidate clarity fields (R-Clarity)

Three enhancements were added to make screener candidates immediately intelligible without technical analysis knowledge.

**`plain_english_summary` (AI-generated)**

A 2–3 sentence plain-English description of what the stock has been doing and why it looks interesting. Written for a reader with no knowledge of technical analysis — no jargon, no signal names. Generated by the same AI batch call in `_batch_enrich_reasoning_via_ai()` (in `breakout_screener.py`, used by both screeners) via a `PLAIN_ENGLISH:` field in the prompt. Parsed with the same `###TICKER` regex pattern as the other AI fields. Shown as the **first and most prominent element** in the expanded detail row, in larger bold text, above `setup_strength` / `key_risk` / `maturity`.

Token budget for the AI batch call was raised from 280 to 400 per candidate to accommodate the new field.

**`base_low` and `pivot_price` (chart annotations)**

`base_low` — the lowest price reached during the base period. Computed in `_compute_base_stats()` in `breakout_screener.py` and now included in its return dict. Propagated through `initial_candidates` → `pre_candidates` → `final_candidates` in both screeners.

`pivot_price` — the Base Pivot High (BPH), the ceiling of the consolidation range. This is the `bph` value already computed by `_compute_base_pivot_high()` and stored in `initial_candidates`. It is now forwarded as `pivot_price` in `pre_candidates` and `final_candidates`.

Both fields pass through `renderer.py` into the template JSON. In the dashboard, they are rendered as dashed horizontal price lines on the candlestick chart (daily view only): `base_low` in green labelled "Base Low", `pivot_price` in amber labelled "Pivot / Breakout Level". The lines are removed when the user switches to weekly view and recreated on return to daily.

**SMA 150 (chart annotation)**

A 150-day SMA line is rendered on the daily chart in blue (`#58a6ff`), labelled "SMA 150". It is computed client-side in `applyBreakoutChartData` / `applyHGChartData` from the full `c.ohlcv_daily` array (so the SMA has enough history regardless of the selected period filter), then filtered by `filterByPeriod` before being set on the series. Not stored in the screener cache.

**Signal badge tooltips**

Each signal badge now carries a `title` attribute with a one-sentence plain-English explanation. The tooltip text is defined in `BREAKOUT_SIGNAL_TIPS` in `template.html`, mirroring the `BREAKOUT_SIGNAL_LABELS` map. If you add a new signal key, add entries to both maps.
